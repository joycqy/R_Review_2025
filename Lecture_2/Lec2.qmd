...
```{webr-r}
# 1. Select year, month, day, and carrier columns as data.table.
flights[, .(year, month, day, carrier)]
```
:::
...
1. Find the average arrival delay and the average departure delay for flights that departed from JFK in August.
```{webr-r}
flights[origin == "JFK" & month == 8L, .(avg_arr_delay = mean(arr_delay), avg_dep_delay = mean(dep_delay))]
```
:::
...
long_data[, c("subject", "year") := tstrsplit(variable, "_", fixed = TRUE)]

# Drop the old variable column
long_data[, variable := NULL]

# Finally, cast the data back to the original long form
dcast(long_data, student + year ~ subject, value.var = "value")
```
...
```{webr-r}
math_reading_data <- merge(math_data, reading_data, by = c("student", "year"), all.x = TRUE)
```
